...
class ProductsListing extends Component {
  ...
  componentDidMount() {
    ...
    if (brand && brand.length !==0 ) {
      this.setState({brandId: brand, brandDisabled: true})
    }
  ...
  getSpotlight() {
    ...
    if (fetchedFeaturedCarousel && featuredCarousel && featuredCarousel.results.length !==0) {
      const {imageLarge}  = featuredCarousel.results[0];

      return (
      <LazyImages
        defaultImage={BANNER_DEFAULT}
        src={imageLarge ? imageLarge : BANNER_DEFAULT}
  ...
  render() {
    return (
      <div className="products-listing listing">
        ...
        <div className={classnames("brands", { "disabled": this.state.brandDisabled })}>
          <BrandList
