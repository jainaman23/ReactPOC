...
class PrivacyPolicy extends Component {
  ...
  componentDidMount() {
    ...
    const { preview, nid, market } = parsed;
    this.setState({
      market: market
    })
  ...
  getRenderPrivacyPolicy() {
    ...
    if (fetchedPolicy) {
      ...
      if (policy && policy.length !== 0) {
        let marketPolicy = '';
        if (market !== undefined) {
          const content = policy.filter((item) => Number(item.mid) === Number(market));
          marketPolicy = content[0];
          if (content.length === 0) {
            return NoContentFound();
          }
        }
        else {
          marketPolicy = policy[0];
        }
        return (
          <>
            <h1 className="page-title" dangerouslySetInnerHTML={this.createMarkup(marketPolicy.title)}></h1>
            <CardSubtitle tag='h3' className="brand-title text-center">{marketPolicy.market}</CardSubtitle>
            <Col className="description" dangerouslySetInnerHTML={this.createMarkup(marketPolicy.body)}></Col>
          </>
        )
      }
