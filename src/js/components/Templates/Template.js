...
import { DISCONTINUE } from '../../helpers/translations';
...
class Template extends Component {
  ...
  getRenderResults() {
    const { items, type, showBrandTitle, showSubTitle, ...attributes } = this.props;
    let link="#";
        ...
        return (
          <Col
            ...
            <Card className='listing-block'>
              <CardLink
                href={`${link}${item.nid ? item.nid : '#' || item.tid ? item.tid : '#'}`}
              >
                <LazyImages
                  defaultImage={TEMPLATE_DEFAULT}
                  src={item.imageSmall}
                />
                {item.discontinue !== 0 && item.discontinue &&
                  <CardSubtitle tag="h3" className="discontinue">
                    {DISCONTINUE}
                  </CardSubtitle>
                }
              </CardLink>
              <CardTitle tag='h2'>
                <CardLink
                href={`${link}${item.nid ? item.nid : '#' || item.tid ? item.tid : '#'}`}
                dangerouslySetInnerHTML={this.createMarkup(item.displayTitle && item.displayTitle)} />
              </CardTitle>
              {showBrandTitle &&
                (item.brand !== 0 && (
